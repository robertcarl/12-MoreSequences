...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(150.0, 40.0),  # All numbers are approximate.
                rg.Point(135.0, 66.0),
                rg.Point(105.0, 66.0),
                rg.Point(90.0, 40.0),
                rg.Point(105.0, 14.0),
                rg.Point(135.0, 14.0)]
    circle = rg.Circle(rg.Point(120, 40), 30)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(500, 300), 200)
    circle.outline_thickness = 5
    polygon(window, circle, 7, 'green', 15)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    for k in range(len(points)):
        poly = rg.Line(points[k-1], points[k])
        poly.attach_to(window)
        poly.color = color
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  20 orange lines on blue circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 255), 200)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 100, 61, 'orange', 2)
    window.close_on_mouse_click()
